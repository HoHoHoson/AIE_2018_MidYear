...
void SpinnySpaceRocksApp::draw() {
	...
	m_2dRenderer->drawSprite(m_shipTexture, m_shipPosX, m_shipPosY, NULL, NULL, m_shipRotation, NULL, 0.5f, 0.5f);
	for (size_t i = 0; i < m_asteroidCount; ++i)
		m_2dRenderer->drawSprite(m_asteroidTexture, m_spaceRocks[i].m_asteroidPosX, m_spaceRocks[i].m_asteroidPosY, NULL, NULL, m_asteroidRotation);
...
{
	int asteroidWidth = m_asteroidTexture->getWidth() / 2;
	int asteroidHeight = m_asteroidTexture->getHeight() / 2;
	int shipWidth = m_shipTexture->getWidth() / 2;
	int shipHeight = m_shipTexture->getHeight() / 2;
